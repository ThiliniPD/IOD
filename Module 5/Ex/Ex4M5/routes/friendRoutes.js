...
router.get('/info', (req, res) => {
    ...
    res.json({browser:req.headers["user-agent"], types:req.headers["accept"]})  
})
...
router.get('/:id', (req, res) => {
    ...
    if (foundFriend != undefined) {
        ...
        res.status(404).json({error: "No friends matching friendId " + friendId})
    }
})
...
router.put('/:id', (req, res) => {
    ...
    let ok = (updatedFriend.name != undefined)
    ok = ok && (updatedFriend.gender == "male" || updatedFriend.gender == "female")

    if(!ok) {
        res.status(500).json({error: 'Friend object must contain a name and a valid gender'});
        return
    }

    updatedFriend.id = friendId
    updatedFriend.gender = updatedFriend.gender.toLowerCase()

    foundFriendIndex = friends.findIndex(friend => friend.id == friendId)
    ...
    if (foundFriendIndex == -1) {
        ...
        friends[foundFriendIndex] = updatedFriend
    }
